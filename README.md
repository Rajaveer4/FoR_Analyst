Project Description (FoR Analyst – Deepfake Audio Detection Website)
...
The website features a complete authentication system with Sign Up, Login, and Dashboard functionalities, handled using browser-based local storage for demonstration purposes. Once logged in, the user gains access to a dedicated dashboard that includes an intuitive audio upload and analysis module. Users can drag and drop audio files or select them manually in .wav format.

Upon uploading an audio file, the platform simulates a backend prediction process that analyzes various voice features. The system provides a prediction result (Real or Fake) and a confidence score, helping users understand the authenticity of the audio clip. A loading animation is included to enhance the user experience during analysis.
